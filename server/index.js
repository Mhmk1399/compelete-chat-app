...
function debugLog(...args) {
  ...
  logger.debug(String(event), rest.length === 1 ? rest[0] : rest.length > 1 ? rest : undefined);
}
...
// dev: coloured single-line  |  production: Apache combined (JSON via Winston)
app.use(
  morgan(isProduction ? "combined" : "dev", {
    stream: logger.morganStream,
    // Skip health-check noise and SSE keep-alives
    skip: (req) =>
      req.path === "/api/health" || req.path === "/api/events",
  }),
);
...
app.listen(port, () => {
  logger.info(`Cheetah Chat server running on http://localhost:${port}`, {
    env: appEnv,
    debug: APP_DEBUG,
  });
});
