...
const backupNamePattern = /^cheetah-chat-backup-.*\.zip$/i;
const serviceName = process.env.CHEETAH_CHAT_SERVICE_NAME || "cheetah-chat.service";
const serviceUser = process.env.CHEETAH_CHAT_SERVICE_USER || "cheetah-chat";
const serviceGroup = process.env.CHEETAH_CHAT_SERVICE_GROUP || serviceUser;
...
function detectBackupLayout(extractedRoot) {
  ...
  const currentDbSrc = path.join(currentDataSrc, "cheetah-chat.db");
  ...
  const legacyDbSrc = path.join(extractedRoot, "cheetah-chat.db");
...
function restartCheetah ChatService() {
  if (typeof process.getuid !== "function" || process.getuid() !== 0) {
    console.warn(
...
async function main() {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cheetah-chat-restore-"));
  try {
    ...
    if (!layout) {
      console.error(
        "Backup zip does not contain expected cheetah-chat.db and uploads/ contents.",
      );
      process.exit(1);
    }
    ...
    const dbDest = path.join(dataDest, "cheetah-chat.db");
