...
const dbPath = path.join(dataDir, "cheetah-chat.db");
...
async function main() {
  ...
  if (!fs.existsSync(dbPath) && !fs.existsSync(uploadsDir)) {
    console.error(
      `No data found in ${dataDir}. Missing cheetah-chat.db and uploads/.`,
    );
    process.exit(1);
  }
  ...
  const backupPath = path.join(backupDir, `cheetah-chat-backup-${stamp}.zip`);
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cheetah-chat-backup-"));

  try {
    fs.mkdirSync(path.join(tempDir, "data"), { recursive: true });
    fs.copyFileSync(envPath, path.join(tempDir, ".env"));
    if (fs.existsSync(dbPath)) {
      fs.copyFileSync(dbPath, path.join(tempDir, "data", "cheetah-chat.db"));
    }
