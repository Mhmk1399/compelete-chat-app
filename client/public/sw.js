...
const CACHE_NAME = `cheetah-chat-${CACHE_VERSION}`;
const APP_SHELL = [
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        keys
          .filter((key) => key.startsWith("cheetah-chat-") && key !== CACHE_NAME)
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    payload = { title: "Cheetah Chat", body: event.data?.text?.() || "" };
  }
  const title = payload.title || "Cheetah Chat";
