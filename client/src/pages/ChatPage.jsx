...
export default function ChatPage({ user, setUser, isDark, setIsDark, toggleTheme }) {
  ...
  useEffect(() => {
    ...
    if (activeChatId) {
      window.dispatchEvent(new Event("cheetah-chat-hide-install-bar"));
    } else {
      window.dispatchEvent(new Event("cheetah-chat-show-install-bar"));
    }
  ...
  const PERMISSION_DISMISS_PREFIX = "cheetah-chat-permission-dismiss-";
  ...
  const handleOpenForwardOrigin = async (target) => {
    ...
    setMentionProfile({
      ...
      color: targetChat.group_color || "#00dafd",
  ...
  useEffect(() => {
    ...
    document.title =
      totalUnreadCount > 0
        ? `Cheetah Chat | ${totalUnread} new message${totalUnread === 1 ? "" : "s"}`
        : "Cheetah Chat";
  ...
  const userColor = user?.color || "#00dafd";
  ...
  const mentionProfileUser =
    mentionProfile?.kind === "user"
      ? {
          ...
          color: mentionProfile.color || "#00dafd",
          status: "online",
        }
      : null;
    ...
    try {
      const res = await forwardMessage({
        ...
        forwardedFromColor: isActiveChannelChat
          ? ""
          : String(
              forwardMessageTarget?.color || user?.color || "#00dafd",
            ).trim(),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    if (liveMentionProfileChat) {
      setMentionProfile((prev) => {
        ...
        const nextColor = liveMentionProfileChat.group_color || prev.color || "#00dafd";
        const nextAvatarUrl =
      ...
      try {
        ...
        if (res.ok) {
          setMentionProfile((prev) => {
            ...
            const nextColor = data?.color || prev.color || "#00dafd";
      ...
      {newGroupOpen ? (
        <Suspense fallback={null}>
          <NewGroupModal
            ...
            avatarColor={editingGroup ? activeChat?.group_color || "#00dafd" : "#00dafd"}
            avatarName={
